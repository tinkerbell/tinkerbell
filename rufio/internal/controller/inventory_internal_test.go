...
// no device — must not panic applyOutOfBandAttributes's idempotency-guard comparison.
...
// successful collection must leave the stored subtree alone. Applying an empty
// object instead would discard last-known-good inventory and take ownership of
// status.attributes, the parent any future sibling subtree also lives under.
func TestApplyOutOfBandAttributesNilDevicePreservesExisting(t *testing.T) {
	scheme := runtime.NewScheme()
	if err := tinkerbell.AddToScheme(scheme); err != nil {
		t.Fatalf("failed to build scheme: %v", err)
	}
	collected := metav1.Now()
	hw := &tinkerbell.Hardware{
		ObjectMeta: metav1.ObjectMeta{Name: "test-hw", Namespace: "test-namespace"},
