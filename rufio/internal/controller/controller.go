...
func NewManager(cfg *rest.Config, opts controllerruntime.Options, powerCheckInterval time.Duration, httpProxyURL string) (controllerruntime.Manager, error) {
	if opts.Scheme == nil {
	...
	if err := NewReconciler(mgr.GetClient()).SetupWithManager(context.Background(), mgr, NewClientFunc(time.Minute, httpProxyURL), powerCheckInterval); err != nil {
