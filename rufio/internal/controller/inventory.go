...
func (r *MachineReconciler) applyOutOfBandAttributes(ctx context.Context, hw *tinkerbell.Hardware, device *common.Device, collectionMethod string) error {
	...
	// A provider that returns (nil, nil) from Inventory() leaves us nothing to
	// write. Skip the apply entirely rather than sending an empty subtree: that
	// would both discard the last-known-good inventory and, because the patch
	// would still name status.attributes, take field-manager ownership of the
	// parent object that any future sibling subtree also lives under.
	if newInventory == nil {
		return nil
	}
