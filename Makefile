...
helm-template: ## Helm template for Tinkerbell
	...
	helm template test helm/tinkerbell --set "trustedProxies={127.0.0.1/24}" --set "artifactsFileServer=http://2.2.2.2" --set "deployment.envs.smee.dhcpv6DefaultNameServers={2001:db8::53,2001:db8::54}" | grep -A1 "name: TINKERBELL_DHCPV6_DEFAULT_NAME_SERVERS" | grep -F -q 'value: "2001:db8::53,2001:db8::54"'
	helm template test helm/tinkerbell --set "trustedProxies={127.0.0.1/24}" --set "artifactsFileServer=http://2.2.2.2" --set "deployment.envs.smee.dhcpv6DefaultDomainSearchList={example.com,lab.example.com}" | grep -A1 "name: TINKERBELL_DHCPV6_DEFAULT_DOMAIN_SEARCH_LIST" | grep -F -q 'value: "example.com,lab.example.com"'
