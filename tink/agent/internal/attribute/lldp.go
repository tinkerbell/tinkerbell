...
// listing it with an empty neighbor list, so there's no way to tell "still
// waiting" apart from "networkd doesn't know about this interface" without
// an independently sourced list of interfaces to wait on. Returns nil
...
func DiscoverLLDP(ctx context.Context, l logr.Logger, timeout time.Duration, ifaces []*data.Network) map[string]*data.LLDPNeighbor {
	...
	neighbors, ok := discoverLLDPViaNetworkd(ctx, l, interfaceNames(ifaces))
	if !ok {
...
// interfaceNames returns the names of ifaces worth waiting on for an LLDP
// neighbor: non-empty, and backed by physical hardware. Virtual interfaces
// (bridges, veth pairs, tunnels, bonds, ...) never receive a real switch's
// LLDP advertisement, but some report carrier up anyway (e.g. a bridge with
// an enslaved port already up) - treating them as candidates would make
// discovery block for up to DefaultLLDPTimeout waiting for a neighbor that
// can never arrive.
func interfaceNames(ifaces []*data.Network) []string {
	var names []string
	for _, iface := range ifaces {
		if iface == nil || iface.Name == nil || *iface.Name == "" {
			continue
		}
...
// isVirtualInterface reports whether name is backed by a virtual device
// (bridge, veth pair, tunnel, bond, ...) rather than physical hardware, via
// the same /sys/class/net/<name> symlink check ghw (this package's own
// hardware-discovery library, see DiscoverNetworks) uses to set its own
// NIC.IsVirtual - a flag DiscoverNetworks doesn't propagate onto
// data.Network, so it has to be re-derived here. Var rather than a plain
// function so tests can fake it without depending on real interfaces. Any
// error (interface missing) is treated as not virtual - the safe default,
// since a missing interface is excluded by the carrier check anyway.
var isVirtualInterface = func(name string) bool {
	dest, err := os.Readlink(filepath.Join("/sys/class/net", name))
	if err != nil {
		return false
	}
	return strings.Contains(dest, "devices/virtual/net")
}
...
// networkd doesn't know about it - callers that need to distinguish "no
// neighbor yet" from "not networkd's concern" must track candidate
// interfaces independently rather than relying on this response. ok is false
// if networkctl couldn't be run at all (e.g. HookOS's LinuxKit base has no
...
// moments ago. It keeps polling as long as any of candidates still lacking a
...
// candidates must be sourced independently of networkctl's response (e.g.
// from a prior DiscoverAll call): a real `networkctl lldp` omits an
// interface entirely until it has a neighbor, so the response alone can't
// tell "still waiting" apart from "not networkd's concern". Interfaces with
// nothing plugged in are excluded from that wait, or a host with any down
// NIC would always block for the full timeout even though no neighbor was
...
func discoverLLDPViaNetworkd(ctx context.Context, l logr.Logger, candidates []string) (map[string]*data.LLDPNeighbor, bool) {
	...
	for {
		...
		if !anyCarrier(pendingInterfaces(candidates, neighbors)) {
			return neighbors, true
		}
		select {
		case <-ctx.Done():
