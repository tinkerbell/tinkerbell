...
func TestDiscoverLLDPDisabledByDefault(t *testing.T) {
	if got := DiscoverLLDP(context.Background(), logr.Discard(), 0, nil); got != nil {
...
// withIsVirtualInterface replaces isVirtualInterface for the duration of the
// test, so tests don't depend on real interfaces existing on the machine
// running them.
func withIsVirtualInterface(t *testing.T, fn func(name string) bool) {
	t.Helper()
	orig := isVirtualInterface
	isVirtualInterface = fn
	t.Cleanup(func() { isVirtualInterface = orig })
}
...
// trimmed to the fields queryNetworkctlLLDP reads. The eno2 entry (still
// awaiting a neighbor) is synthetic: a real `networkctl lldp` call querying
...
func TestQueryNetworkctlLLDPInvalidJSON(t *testing.T) {
	...
	if _, ok := queryNetworkctlLLDP(context.Background(), logr.Discard()); ok {
		t.Error("queryNetworkctlLLDP ok = true, want false on unparseable output")
	}
}

// TestInterfaceNamesExcludesVirtualInterfaces guards against treating a
// bridge/veth/bond/tunnel as an LLDP candidate: such interfaces never get a
// real neighbor, but some report carrier up anyway (e.g. a bridge with an
// enslaved port already up), which would otherwise make discovery block for
// the full DefaultLLDPTimeout for nothing.
func TestInterfaceNamesExcludesVirtualInterfaces(t *testing.T) {
	withIsVirtualInterface(t, func(name string) bool { return name == "docker0" })
	...
	got := interfaceNames(ifaces)
	if want := []string{"eno1"}; len(got) != 1 || got[0] != want[0] {
...
func TestDiscoverLLDPViaNetworkdReturnsImmediatelyWhenFound(t *testing.T) {
	...
	neighbors, ok := discoverLLDPViaNetworkd(context.Background(), logr.Discard(), []string{"eno1", "eno2"})
	if !ok {
...
func TestDiscoverLLDPViaNetworkdPollsUntilFound(t *testing.T) {
	...
	withNetworkctlLLDP(t, func(context.Context) ([]byte, error) {
		calls++
		if calls < 3 {
			return []byte(`{"Neighbors":[]}`), nil // real networkctl shape: nothing found yet
		}
		return []byte(networkctlSampleAllFound), nil
	})
	...
	defer cancel()
	neighbors, ok := discoverLLDPViaNetworkd(ctx, logr.Discard(), []string{"eno1", "eno2"})
	if !ok {
		t.Fatal("discoverLLDPViaNetworkd ok = false, want true")
	}
	if len(neighbors) != 2 {
...
func TestDiscoverLLDPViaNetworkdPollsForSlowerInterfaces(t *testing.T) {
	...
	withNetworkctlLLDP(t, func(context.Context) ([]byte, error) {
		calls++
		if calls < 3 {
			return []byte(networkctlSample), nil // eno1 found, eno2 still pending
		}
		return []byte(networkctlSampleAllFound), nil
	})
...
// NICs (e.g. spare ports, an IPMI-only interface): they will never get an
// LLDP neighbor. Waiting the full timeout on them on every boot would be a
// needless startup delay, so discovery must return as soon as the only
// candidates still missing a neighbor are down.
...
// candidate (never gets a neighbor) with one up candidate (never gets a
...
// TestDiscoverLLDPViaNetworkdPollsForInterfaceOmittedFromResponse proves the
// high-severity bug flagged by Copilot's review of this PR
// (github.com/tinkerbell/tinkerbell/pull/944): a real `networkctl lldp
// --json=short` call (no interface filter) never reports an interface that
// has no neighbor yet - systemd's vl_method_get_lldp_neighbors skips it
// entirely (see HASHMAP_FOREACH + `if (sd_json_variant_is_blank_array(v))
// continue;` in networkd-manager-varlink.c) rather than including it with an
// empty inner "Neighbors" list. Discovery must therefore track candidate
// interfaces independently (via the candidates parameter, sourced from a
// prior DiscoverAll call) rather than relying on networkctl's response to
// tell it what's still pending. This mirrors the actual raw sample captured
// in PR comment 5424795070 (a single-interface response, no zero-neighbor
// entry alongside it), not the synthetic "eno2" fixture used elsewhere in
// this file.
func TestDiscoverLLDPViaNetworkdPollsForInterfaceOmittedFromResponse(t *testing.T) {
	orig := networkdPollInterval
	networkdPollInterval = time.Millisecond
	t.Cleanup(func() { networkdPollInterval = orig })
	withHasCarrier(t, func(string) bool { return true })
	...
	withNetworkctlLLDP(t, func(context.Context) ([]byte, error) {
		calls++
		if calls < 3 {
			// eno1 has carrier but hasn't advertised yet: a real
			// `networkctl lldp` omits it entirely rather than
			// listing it with an empty Neighbors array.
			return []byte(`{"Neighbors":[]}`), nil
		}
		return []byte(`{"Neighbors":[{"InterfaceName":"eno1","Neighbors":[{"ChassisID":"aa","SystemName":"switch01"}]}]}`), nil
	})

	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	neighbors, ok := discoverLLDPViaNetworkd(ctx, logr.Discard(), []string{"eno1"})
	if !ok {
		t.Fatal("discoverLLDPViaNetworkd ok = false, want true")
	}
	if calls != 3 {
		t.Errorf("networkctl called %d times, want 3 (should have kept polling for eno1 instead of stopping after the first empty response)", calls)
	}
	if _, found := neighbors["eno1"]; !found {
		t.Errorf("neighbors = %+v, want an eno1 entry once it advertised", neighbors)
	}
}

// TestDiscoverLLDPViaNetworkdKeepsLastResultOnTransientError proves the
// medium-severity bug flagged by Copilot's review of this PR: a networkctl
// failure on a later poll (e.g. the command erroring, or the context
// expiring mid-invocation) must not discard neighbors already found by an
// earlier successful poll.
func TestDiscoverLLDPViaNetworkdKeepsLastResultOnTransientError(t *testing.T) {
	orig := networkdPollInterval
	networkdPollInterval = time.Millisecond
	t.Cleanup(func() { networkdPollInterval = orig })
	withHasCarrier(t, func(string) bool { return true })
	...
	withNetworkctlLLDP(t, func(context.Context) ([]byte, error) {
		...
		return nil, errors.New("networkctl: transient failure")
	})
	...
	neighbors, ok := discoverLLDPViaNetworkd(ctx, logr.Discard(), []string{"eno1", "eno2"})
	if !ok {
		t.Fatal("discoverLLDPViaNetworkd ok = false, want true (an earlier query succeeded)")
	}
	if _, found := neighbors["eno1"]; !found {
		t.Errorf("neighbors = %+v, want the eno1 entry found by the first successful query, not discarded by the later failure", neighbors)
	}
}
...
func TestDiscoverLLDPViaNetworkdFallsBackImmediatelyOnError(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	if _, ok := discoverLLDPViaNetworkd(ctx, logr.Discard(), nil); ok {
...
func TestDiscoverLLDPReturnsNeighborsFromNetworkd(t *testing.T) {
	...
	name := func(s string) *string { return &s }
	ifaces := []*data.Network{{Name: name("eno1")}}
	neighbors := DiscoverLLDP(context.Background(), logr.Discard(), time.Second, ifaces)
	if len(neighbors) != 1 {
...
func TestDiscoverLLDPSkipsGracefullyWithoutSystemd(t *testing.T) {
	...
	if got := DiscoverLLDP(context.Background(), logr.Discard(), time.Second, nil); got != nil {
