...
func TestConfig_syslogHostV6(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			syslogFQDN: "syslog-v6.example.com",
...
func TestDHCPv6HandlerAutoStatelessEnabled(t *testing.T) {
	...
	if diff := cmp.Diff([]net.IP{net.ParseIP("2001:db8::53")}, h.DNSDefaults.NameServers); diff != "" {
		t.Fatalf("unexpected DHCPv6 default nameservers (-want +got):\n%s", diff)
	}
	if diff := cmp.Diff([]string{"example.com"}, h.DNSDefaults.DomainSearch); diff != "" {
		t.Fatalf("unexpected DHCPv6 default domain search list (-want +got):\n%s", diff)
	}
}
...
func TestScriptHandlerUsesStaticIPXEForDHCPv6AutoStateless(t *testing.T) {
	...
	cfg.IPXE.HTTPScriptServer.SyslogFQDN = "2001:db8::20"
	...
	} else if !strings.Contains(body, "set syslog6 2001:db8::20") {
...
func TestScriptHandlerPreservesIPv4AutoProxyFallbackWhenDHCPDisabled(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"IPv4 fallback remains enabled": {
			...
			wantBody:   "set syslog 192.0.2.10",
		},
