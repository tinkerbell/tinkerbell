...
func TestAutoProxyEnabled(t *testing.T) {
	t.Skip()
	lo, _ := net.InterfaceByName("lo")
	ip := netip.MustParseAddr("127.0.0.1")
	binServerTFTP := netip.AddrPortFrom(ip, 69)
	...
	// Create a test for AutoProxyEnabled
	h := Handler{
		Log:              logr.Discard(),
		Backend:          &mockBackend{err: errBackend}, // Backend will error, but AutoProxyEnabled will bypass
		Netboot:          Netboot{Enabled: true, IPXEBinServerTFTP: binServerTFTP, IPXEBinServerHTTP: binServerHTTP, IPXEScriptURL: ipxeScript},
		IPAddr:           ip,
		AutoProxyEnabled: true, // Key setting: enables auto proxy mode
	}
	...
	peer := &net.UDPAddr{IP: net.IPv4(127, 0, 0, 1), Port: pc.LocalAddr().(*net.UDPAddr).Port}
	...
	// Create a valid netboot client packet
	pkt := &dhcpv4.DHCPv4{
		OpCode:       dhcpv4.OpcodeBootRequest,
		ClientHWAddr: []byte{1, 2, 3, 4, 5, 6},
		Options: dhcpv4.OptionsFromList(
	...
	// Should work despite backend error due to AutoProxyEnabled
	h.Handle(context.Background(), con, dhcp.Packet{
		Peer: peer,
		Pkt:  pkt,
		Md:   &dhcp.Metadata{IfName: lo.Name, IfIndex: lo.Index},
	})
	...
	msg, err := clientResponse(pc)
	if err != nil {
		t.Fatal("expected response with AutoProxyEnabled but got error:", err)
	}

	// Verify essential response fields
	if msg.OpCode != dhcpv4.OpcodeBootReply {
		t.Fatalf("expected OpCode %v, got %v", dhcpv4.OpcodeBootReply, msg.OpCode)
	}

	if msg.MessageType() != dhcpv4.MessageTypeOffer {
		t.Fatalf("expected MessageType %v, got %v", dhcpv4.MessageTypeOffer, msg.MessageType())
	}

	if msg.BootFileName != "http://localhost/script.ipxe" {
		t.Fatalf("expected BootFileName %v, got %v", "http://localhost/script.ipxe", msg.BootFileName)
	}
}
