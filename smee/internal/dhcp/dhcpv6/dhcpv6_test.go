...
func TestApplyRequestedStatelessOptionsDNSDefaults(t *testing.T) {
	request := messageWithDUIDEN(t, dhcpv6.MessageTypeInformationRequest)
	dhcpv6.WithRequestedOptions(
		dhcpv6.OptionDNSRecursiveNameServer,
		dhcpv6.OptionDomainSearchList,
	...
	tests := map[string]struct {
		dhcp             *dhcp.DHCP
		wantNameServers  []net.IP
		wantDomainSearch []string
	}{
		"nil hardware data uses defaults": {
			wantNameServers:  []net.IP{net.ParseIP("2001:db8::53")},
			wantDomainSearch: []string{"default.example.com"},
		},
		"unusable family uses defaults": {
			dhcp: &dhcp.DHCP{
				NameServers: []net.IP{net.ParseIP("192.0.2.99")},
			},
			wantNameServers:  []net.IP{net.ParseIP("2001:db8::53")},
			wantDomainSearch: []string{"default.example.com"},
		},
		"hardware overrides defaults": {
			dhcp: &dhcp.DHCP{
				NameServers:  []net.IP{net.ParseIP("192.0.2.99"), net.ParseIP("2001:db8::99")},
				DomainSearch: []string{"hardware.example.com"},
			},
			wantNameServers:  []net.IP{net.ParseIP("2001:db8::99")},
			wantDomainSearch: []string{"hardware.example.com"},
		},
		"hardware domain name overrides default domain search": {
			dhcp: &dhcp.DHCP{
				DomainName: "hardware.example.com",
			},
			wantNameServers:  []net.IP{net.ParseIP("2001:db8::53")},
			wantDomainSearch: []string{"hardware.example.com"},
		},
		"nameservers fall back independently": {
			dhcp: &dhcp.DHCP{
				NameServers:  []net.IP{net.ParseIP("192.0.2.99")},
				DomainSearch: []string{"hardware.example.com"},
			},
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			reply := &dhcpv6.Message{}
			ApplyRequestedStatelessOptions(reply, Info{Msg: request}, tt.dhcp, defaults)
			if diff := cmp.Diff(tt.wantNameServers, reply.Options.DNS()); diff != "" {
				t.Fatalf("unexpected nameservers (-want +got):\n%s", diff)
			}
			if diff := cmp.Diff(tt.wantDomainSearch, reply.Options.DomainSearchList().Labels); diff != "" {
