...
// DNSDefaults are fallback DNS settings used when Hardware does not provide
// values usable by DHCPv6.
...
func ApplyRequestedStatelessOptions(reply dhcpv6.DHCPv6, i Info, d *dhcp.DHCP, defaults DNSDefaults) {
	var nameServers, ntpServers []net.IP
	var domainSearch []string
	if d != nil {
		nameServers = IPv6Only(d.NameServers)
		ntpServers = IPv6Only(d.NTPServers)
		domainSearch = d.DomainSearch
		if len(domainSearch) == 0 && d.DomainName != "" {
			domainSearch = []string{d.DomainName}
		}
	}
	if len(nameServers) == 0 {
		nameServers = IPv6Only(defaults.NameServers)
	}
	if len(domainSearch) == 0 {
		domainSearch = defaults.DomainSearch
	}

	if i.Msg.IsOptionRequested(dhcpv6.OptionDNSRecursiveNameServer) && len(nameServers) > 0 {
		dhcpv6.WithDNS(nameServers...)(reply)
	}
	if i.Msg.IsOptionRequested(dhcpv6.OptionDomainSearchList) && len(domainSearch) > 0 {
		dhcpv6.WithDomainSearchList(domainSearch...)(reply)
	}
	if i.Msg.IsOptionRequested(dhcpv6.OptionNTPServer) && len(ntpServers) > 0 {
