...
func Intn(mx int) int {
	rng.Lock()
	defer rng.Unlock()
	return rng.rand.Intn(mx)
}

// IntnRange generates an integer in range [min,max).
// By design this should panic if input is invalid, <= 0.
func IntnRange(mn, mx int) int {
	rng.Lock()
	defer rng.Unlock()
	return rng.rand.Intn(mx-mn) + mn
}

// IntnRange generates an int64 integer in range [min,max).
// By design this should panic if input is invalid, <= 0.
func Int63nRange(mn, mx int64) int64 {
	rng.Lock()
	defer rng.Unlock()
	return rng.rand.Int63n(mx-mn) + mn
}
