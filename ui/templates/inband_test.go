...
func TestAgentAttributesFromInBand(t *testing.T) {
	attrs := &tinkv1alpha1.Attributes{
		CPU: &tinkv1alpha1.CPU{
			...
			Sockets: []tinkv1alpha1.CPUSocket{
				{Slot: "2", Vendor: "GenuineIntel", Model: "i7", Cores: 8, Threads: 16, Capabilities: []string{"fpu", "vme"}},
			},
		},
	...
	if len(got.CPU.Processors) != 1 || got.CPU.Processors[0].Model != "i7" || len(got.CPU.Processors[0].Capabilities) != 2 || got.CPU.Processors[0].ID != 2 {
		t.Errorf("CPU.Processors = %+v, want one entry with ID=2, Model=i7, and 2 capabilities", got.CPU.Processors)
	}

	if got.Memory.Total != "7.5 GiB" || got.Memory.Usable != "7.0 GiB" {
...
func TestAgentAttributesFromInBandNonNumericSlotFallsBackToIndex(t *testing.T) {
	// Out-of-band reports Slot as a physical socket label (e.g. "CPU1"), not a
	// number; when it doesn't parse, the array index is used instead of erroring
	// or leaving a zero-value ID for every socket.
	attrs := &tinkv1alpha1.Attributes{
		CPU: &tinkv1alpha1.CPU{
			Sockets: []tinkv1alpha1.CPUSocket{
				{Slot: "CPU1", Model: "i7"},
	...
	got := AgentAttributesFromInBand(attrs)
	if len(got.CPU.Processors) != 2 {
		t.Fatalf("CPU.Processors = %+v, want two entries", got.CPU.Processors)
	}
	if got.CPU.Processors[0].ID != 0 || got.CPU.Processors[1].ID != 1 {
		t.Errorf("CPU.Processors IDs = [%d, %d], want [0, 1] (fallback to index)", got.CPU.Processors[0].ID, got.CPU.Processors[1].ID)
	}
}
